...
export async function fetchVoteResults(
  ...
  try {
    ...
    console.log(
      "getMultipleAccountsInfo returned:",
      accountInfos.map((info) => !!info?.data)
    );
    ...
    if (accountInfos[1]?.data) {
      try {
        const distributorAccountData = AccountLayout.decode(
          accountInfos[1].data
        );
    ...
    const participationRate =
      totalSupply > 0
        ? Number((totalVotes * BigInt(10000)) / totalSupply) / 100
        : 0;
    const claimRate =
      totalSupply > 0
        ? Number((totalClaimed * BigInt(10000)) / totalSupply) / 100
        : 0;
...
export function calculateUserVotingPower(
  ...
  const percentage =
    Number((userAllocation * BigInt(1000000)) / totalSupply) / 10000;
...
export function formatVotePercentage(
  votes: bigint,
  totalVotes: bigint
): string {
