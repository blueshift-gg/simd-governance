...
export function findNodeForAddress(merkleData: MerkleTreeData, address: PublicKey): TreeNode | null {
...
export function calculateAmountUnlocked(node: TreeNode): bigint {
  return BigInt(node.total_unlocked_searcher) +
...
export function calculateAmountLocked(node: TreeNode): bigint {
  return BigInt(node.total_locked_searcher) +
