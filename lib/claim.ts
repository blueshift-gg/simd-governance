...
import { TreeNode, calculateAmountLocked, calculateAmountUnlocked } from "./merkle-tree"
import { getMerkleDistributorPDA, getClaimStatusPDA } from "./pda"
import { MERKLE_DISTRIBUTOR_PROGRAM_ID, TOKEN_MINT, AIRDROP_VERSION } from "./constants"
...
function serializeNewClaimInstruction(data: NewClaimInstructionData): Buffer {
  ...
  const discriminator = Buffer.from([0x4e, 0xb1, 0x62, 0x7b, 0xd2, 0x15, 0xbb, 0x53])
  
  // Serialize amount_unlocked (8 bytes, little-endian) 
  const amountUnlockedBuffer = Buffer.allocUnsafe(8)
  amountUnlockedBuffer.writeBigUInt64LE(BigInt(data.amount_unlocked.toString()))
...
export async function buildClaimTransaction(
  ...
  const proof = node.proof.map(proofElement => {
    ...
    if (proofElement.length !== 32) {
      throw new Error(`Invalid proof element length: ${proofElement.length}, expected 32`)
    }
  ...
  if (priorityFee && priorityFee > 0) {
    ...
    console.log(`Added priority fee instruction of ${priorityFee} microlamports`)
  }
