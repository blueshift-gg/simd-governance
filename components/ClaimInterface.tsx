...
export default function ClaimInterface() {
  ...
  const [userVotingPower, setUserVotingPower] =
    useState<UserVotingPower | null>(null);
  ...
  useEffect(() => {
    if (publicKey && merkleData) {
      ...
      checkClaimStatus(connection, publicKey)
        .then((claimed) => {
    ...
    const loadVoteData = async () => {
      ...
      try {
        const results = await fetchVoteResults(
          ...
          VOTE_ADDRESSES.YES,
          VOTE_ADDRESSES.NO,
          VOTE_ADDRESSES.ABSTAIN
        );
        setVoteResults(results);

        if (userNode) {
          const userAllocation =
            calculateAmountUnlocked(userNode) + calculateAmountLocked(userNode);
          const votingPower = calculateUserVotingPower(
            userAllocation,
            results.totalSupply
          );
  ...
  const handleVote = useCallback(async () => {
    ...
    const amountBN = BigInt(Math.floor(amount * LAMPORTS_PER_SOL));
    ...
    const voteAccounts = {
      yes: new PublicKey(VOTE_ADDRESSES.YES),
      no: new PublicKey(VOTE_ADDRESSES.NO),
      abstain: new PublicKey(VOTE_ADDRESSES.ABSTAIN),
    };
    ...
    try {
      ...
      const voteAta = await getAssociatedTokenAddress(
        TOKEN_MINT,
        voteAccount,
        true,
        TOKEN_PROGRAM_ID
  ...
  const voteChartData = voteResults
    ? [
        ...
        {
          name: "ABSTAIN",
          value: Number(voteResults.abstainVotes),
          color: "#2A2B30",
        },
      ].filter((d) => d.value > 0)
    : [];
    ...
    ? [
        {
          name: "CLAIMED",
          value:
            (Number(voteResults.totalClaimed) /
              Number(voteResults.totalSupply)) *
            100,
          fill: "#00FFFF",
        },
        {
        ...
        },
        {
          name: "UNCLAIMED",
          value:
            (Number(voteResults.distributorBalance) /
              Number(voteResults.totalSupply)) *
            100,
          fill: "#2A2B30",
        },
  ...
  useEffect(() => {
    const fetchEpochInfo = async () => {
      try {
        ...
        // Calculate estimated end time of voting
        if (info.epoch <= VOTING_END_EPOCH) {
          const epochsRemaining = VOTING_END_EPOCH - info.epoch;
          const slotsRemaining =
            epochsRemaining * BLOCKS_PER_EPOCH +
            (info.slotsInEpoch - info.slotIndex);
  ...
  const votingStatus = voteResults
    ? {
        ...
        yesPercentage:
          Number(voteResults.totalVotes) > 0
            ? (Number(voteResults.yesVotes) / Number(voteResults.totalVotes)) *
              100
            : 0,
        ...
        supermajorityMet:
          Number(voteResults.totalVotes) > 0
            ? Number(voteResults.yesVotes) / Number(voteResults.totalVotes) >=
              2 / 3
            : false,
        votesNeededForQuorum: Math.max(
          0,
          Math.ceil((Number(voteResults.totalSupply) * 0.3333) / 1e9) -
            Number(voteResults.totalVotes) / 1e9
        ),
  ...
  const getVoteStatus = () => {
    if (!votingStatus)
      return {
        status: "LOADING",
        subtext: "",
        color: "text-blueshift-gray-400",
      };

    if (!votingStatus.quorumMet) {
      return {
        status: "PENDING",
        subtext: "PENDING QUORUM",
        color: "text-blueshift-gray-400",
      };
    }
    ...
    if (votingStatus.supermajorityMet) {
      ...
      return {
        status: "PENDING",
        subtext: "PENDING SUPERMAJORITY",
        color: "text-blueshift-blue",
      };
    }
  };
    ...
    <div className="min-h-screen px-4 py-8 bg-gradient-radial from-blueshift-cyan/[0.02] to-blueshift-dark">
      ...
      <header className="max-w-7xl mx-auto mb-8">
        ...
        <div className="text-left">
          <h1 className="text-2xl sm:text-3xl font-bold text-white font-mono uppercase mb-2">
            SIMD-0326:Alpenglow
          </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          >
            ...
            <div className="mb-2 flex-1 flex items-center justify-center gap-2">
              <span
                className={`text-xl font-bold font-mono ${voteStatusInfo.color}`}
              >
                {voteStatusInfo.status}
              </span>
              {votingStatus?.quorumMet && votingStatus?.supermajorityMet && (
                <svg
                  className="w-5 h-5 fill-current text-blueshift-cyan"
                  viewBox="0 0 20 20"
                >
                  <path
                    fillRule="evenodd"
            ...
            <p className="text-xs text-blueshift-gray-500 font-mono">
              {voteStatusInfo.subtext ||
                (votingStatus
                  ? `${votingStatus.yesPercentage.toFixed(1)}% VOTED YES`
                  : "Loading...")}
            </p>
          </div>
          ...
          >
            ...
            <p className="text-xs text-blueshift-gray-500 font-mono">
              {votingStatus
                ? `${votingStatus.participationRate.toFixed(1)}% PARTICIPATED`
                : "Loading..."}
            </p>
          </div>
            ...
            {voteResults && (
              <>
                <div className="flex justify-between text-sm font-mono">
                  <span className="text-blueshift-cyan uppercase tracking-wider">
                    YES
                  </span>
                  <span className="text-blueshift-gray-300">
                    {formatVotePercentage(
                ...
                <div className="flex justify-between text-sm font-mono">
                  <span className="text-blueshift-blue uppercase tracking-wider">
                    NO
                  </span>
                  <span className="text-blueshift-gray-300">
                    {formatVotePercentage(
                ...
                <div className="flex justify-between text-sm font-mono">
                  <span className="text-blueshift-gray-400 uppercase tracking-wider">
                    ABSTAIN
                  </span>
                  <span className="text-blueshift-gray-300">
                    {formatVotePercentage(
                      voteResults.abstainVotes,
                      voteResults.totalVotes
                    )}
                    %
                  </span>
          ...
          {voteResults && (
            <div className="mt-4 space-y-2">
              ...
              <div className="flex justify-between text-sm font-mono">
                ...
                <span className="text-blueshift-gray-300">
                  {voteResults.participationRate.toFixed(1)}%
                </span>
              </div>
              <div className="flex justify-between text-sm font-mono">
                <span className="text-blueshift-gray-400 uppercase tracking-wider">
              ...
              {userNode ? (
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-blueshift-gray-400 font-mono text-sm uppercase tracking-wider">
                      Current Balance
                    </span>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono text-white text-sm">
                      {hasClaimed
                        ? formatAmount(calculateAmountUnlocked(userNode))
                        : "0.000"}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono text-blueshift-gray-500 text-sm">
                      {hasClaimed
                        ? formatAmount(calculateAmountLocked(userNode))
                        : formatAmount(calculateAmountUnlocked(userNode))}
                    </span>
                  </div>
                  ...
                  {checkingClaimStatus ? (
                    <div className="flex items-center justify-center py-4">
                      <svg
                        className="animate-spin h-5 w-5 text-blueshift-cyan mr-3"
                        fill="none"
                        viewBox="0 0 24 24"
                      >
                        <circle
                      ...
                      <span className="text-blueshift-gray-400 font-mono">
            ...
            <div className="glass-card p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-white font-mono text-sm uppercase tracking-wider">
                    Vote Option
                  </label>
                  <div className="flex flex-col sm:flex-row bg-blueshift-gray-800 rounded-lg p-1 gap-1 sm:gap-0">
                    <button
                      type="button"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <div>
                      <span className="text-blueshift-gray-400 font-mono text-xs uppercase tracking-wider">
                        Available:{" "}
                      </span>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setVoteAmount(
                          (
                            Number(userTokenBalance) / LAMPORTS_PER_SOL
                          ).toString()
                        )
                      }
                ...
                >
                  {voting
                    ...
                    : userTokenBalance === BigInt(0)
                    ? "No Tokens Available"
                    : `Submit ${selectedVoteOption.toUpperCase()} Vote`}
                </button>

                <p className="text-xs text-blueshift-gray-500 text-center">
                  {userTokenBalance === BigInt(0)
                    ? "You need tokens to vote. Claim your allocation above if eligible."
